...
var calendarHeatmap = {
  ...
  createElements: function () {
    ...
    // Create svg element
    var svg = d3v3.select(container).append('svg')
      .attr('class', 'svg');
    ...
    calendarHeatmap.tooltip = d3v3.select(container).append('div')
  ...
  drawGlobalOverview: function () {
    ...
    var year_data = d3v3.time.years(start, end).map(function (d) {
      var date = moment(d);
      return {
        'date': date,
    ...
    var max_value = d3v3.max(year_data, function (d) {
      return d.total;
    });

    // Define year labels and axis
    var year_labels = d3v3.time.years(start, end).map(function (d) {
      return moment(d);
    });
    var yearScale = d3v3.scale.ordinal()
    ...
    var item_block = calendarHeatmap.items.selectAll('.item-block-year')
      ...
      .on('mouseover', function(d) {
        ...
        // Construct tooltip
        var tooltip_html = '';
        tooltip_html += '<div><span><strong>Total amount sold:</strong></span>';
  ...
  drawYearOverview: function () {
    ...
    calendarHeatmap.items.selectAll('.item-circle')
      ...
      .on('mouseover', function (d) {
        ...
        // Construct tooltip
        var tooltip_html = '';
        tooltip_html += '<div class="header"><strong>' + (d.total ? calendarHeatmap.formatTime(d.total) : 'No time') + ' items sold</strong></div>';
    ...
    // Add month labels
    var month_labels = d3v3.time.months(start_of_year, end_of_year);
    var monthScale = d3v3.scale.linear()
    ...
    // Add day labels
    var day_labels = d3v3.time.days(moment().startOf('week'), moment().endOf('week'));
    var dayScale = d3v3.scale.ordinal()
  ...
  drawMonthOverview: function () {
    ...
    var max_value = d3v3.max(month_data, function (d) {
      return d3v3.max(d.summary, function (d) {
        return d.value;
      });
    });
    ...
    var weekScale = d3v3.scale.ordinal()
    ...
    var itemScale = d3v3.scale.linear()
    ...
    item_block.selectAll('.item-block-rect')
      ...
      .attr('x', function (d) {
        var total = parseInt(d3v3.select(this.parentNode).attr('total'));
        var offset = parseInt(d3v3.select(this.parentNode).attr('offset'));
        itemScale.domain([0, total]);
        d3v3.select(this.parentNode).attr('offset', offset + itemScale(d.value));
        return offset;
      })
      .attr('width', function (d) {
        var total = parseInt(d3v3.select(this.parentNode).attr('total'));
      ...
      .on('mouseover', function(d) {
        ...
        var date = new Date(d3v3.select(this.parentNode).attr('date'));

        // Construct tooltip
        var tooltip_html = '';
        tooltip_html += '<div class="header"><strong>' + d.name + '</strong></div><br>';
        tooltip_html += '<div><strong>' + (d.value ? calendarHeatmap.formatTime(d.value) : 'No time') + ' items sold</strong></div>';
  ...
  drawWeekOverview: function () {
    ...
    var max_value = d3v3.max(week_data, function (d) {
      return d3v3.max(d.summary, function (d) {
        return d.value;
      });
    });
    ...
    item_block.selectAll('.item-block-rect')
      ...
      .on('mouseover', function(d) {
        ...
        var tooltip_html = '';
        tooltip_html += '<div class="header"><strong>' + d.name + '</strong></div><br>';
        tooltip_html += '<div><strong>' + (d.value ? calendarHeatmap.formatTime(d.value) : 'No time') + ' item sold</strong></div>';
        tooltip_html += '<div>on ' + moment(date).format('dddd, MMM Do YYYY') + '</div>';

        // Calculate tooltip position
        var total = parseInt(d3v3.select(this.parentNode).attr('total'));
        itemScale.domain([0, total]);
        var x = parseInt(d3v3.select(this).attr('x')) + itemScale(d.value) / 4 + calendarHeatmap.settings.tooltip_width / 4;
        while ( calendarHeatmap.settings.width - x < (calendarHeatmap.settings.tooltip_width + calendarHeatmap.settings.tooltip_padding * 3) ) {
          x -= 10;
        }
  ...
  drawDayOverview: function () {
    ...
    var projectScale = d3v3.scale.ordinal()
      .rangeRoundBands([calendarHeatmap.settings.label_padding, calendarHeatmap.settings.height])
      .domain(project_labels);

    var itemScale = d3v3.time.scale()
    ...
    calendarHeatmap.items.selectAll('.item-block')
      ...
      .attr('width', function (d) {
        // var end = itemScale(d3v3.time.second.offset(moment(d.date), d.value));
